...
namespace Nalog
{
    public partial class NalogUrForm : Form
    {
        ...
        private void LoadData()
        {
            ...
            dataAdapter = new SqlDataAdapter(" SELECT idNalogUr, NameOrg as 'Организация' ,NameVidNalog as 'Вид налога', innurlic as 'ИНН юр лица', OblSumm as 'Облагаемая сумма', DateOpovesh as 'Дата получения извещения', DateOplata as 'Дата оплаты', Oplata as 'Оплачено', Oplacheno as 'Оплатил', Dolg as 'Долг' FROM nalogur p INNER JOIN vidnalogur ps ON ps.idVidNalog = idVid INNER JOIN urlica pd ON pd.idUr = idU ORDER BY idNalogUr", dataBaseConnection);
            dataAdapter.Fill(ds, "nalogur");
            DT = ds.Tables["nalogur"];
        ...
        {
            AddUrNalogForm nfrm = new AddUrNalogForm();
            nfrm.Show();
            this.Close();
        }
        ...
        {
            string sql = "UPDATE nalogur SET DateOplata='" + MainTable[6, MainTable.SelectedCells[0].RowIndex].Value.ToString() + "', Oplata='" + MainTable[7, MainTable.SelectedCells[0].RowIndex].Value.ToString() + "', Oplacheno='" + MainTable[8, MainTable.SelectedCells[0].RowIndex].Value.ToString() + "', Dolg='" + MainTable[9, MainTable.SelectedCells[0].RowIndex].Value.ToString()
             + "' WHERE idNalogUr='" + MainTable[0, MainTable.SelectedCells[0].RowIndex].Value.ToString() + "'"; ;
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string commTextt = "delete from nalogur where idNalogUr = @idNalogUr";
                myConnection.Open();
                ...
                commt.Parameters.AddWithValue("@idNalogUr", MainTable[0, MainTable.CurrentRow.Index].Value);
                try
                {
