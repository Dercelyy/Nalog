...
namespace Nalog
{
    public partial class VidFizForm : Form
    {
        ...
        private void LoadData()
        {
            ...
            dataAdapter = new SqlDataAdapter(" SELECT idVidnalog, NameFizNalog as 'Вид налога', opisane as 'Описание' FROM vidnalogfiz", dataBaseConnection);
            dataAdapter.Fill(ds, "vidnalogfiz");
            DT = ds.Tables["vidnalogfiz"];
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SqlCommand createUser = new SqlCommand("INSERT INTO vidnalogfiz (NameFizNalog, opisane)VALUES(@NameFizNalog, @opisane)", sqlConnection);
                sqlConnection.Open();
                createUser.Parameters.AddWithValue("NameFizNalog", NameBox.Text);
                createUser.Parameters.AddWithValue("opisane", OpisBox.Text);
                try
                {
        ...
        {
            string sql = "UPDATE vidnalogfiz SET NameFizNalog='" + MainTable[1, MainTable.SelectedCells[0].RowIndex].Value.ToString() + "', opisane='" + MainTable[2, MainTable.SelectedCells[0].RowIndex].Value.ToString()
                 + "' WHERE idVidnalog='" + MainTable[0, MainTable.SelectedCells[0].RowIndex].Value.ToString() + "'"; ;
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string commTextt = "delete from vidnalogfiz where idVidnalog = @idVidnalog";
                myConnection.Open();
                ...
                commt.Parameters.AddWithValue("@idVidnalog", MainTable[0, MainTable.CurrentRow.Index].Value);
                try
                {
